...
async def ingest(project_id: str = Form(...), files: List[UploadFile] = File(...)) -> IngestResponse:
    ...
    saved: List[str] = []
    for upload in files:
        dest = os.path.join(base, "uploads", upload.filename)
        await stream_save(upload, dest)
        saved.append(dest)
        ...
        if ext == ".pdf":
            ...
            try:
                if ext == ".csv":
                    df = pd.read_csv(dest)
                    parsed_items = [{"page_or_sheet": 1, "text": df.to_csv(index=False)}]
                else:
                    xls = pd.ExcelFile(dest)
                    for sheet in xls.sheet_names:
                        df = xls.parse(sheet)
                        parsed_items.append({"page_or_sheet": sheet, "text": df.to_csv(index=False)})
            except Exception:
                parsed_items = []
...
def generate(req: GenerateRequest) -> JSONResponse:
    ...
    passages = reranker.rerank(hint, vi.search(hint, top_k=TOP_K), k=RERANK_K)
    ...
    for sec in sections:
        ...
        outputs.append({"section": sec, "content": llm.complete(prompt, system=SYSTEM_PROMPT)})
