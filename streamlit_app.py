...
def _parse_sales_ramp(text: str) -> List[float]:
    try:
        ...
        st.sidebar.warning("Sales ramp factors invalid. Using default ramp instead.")
...
def main() -> None:
    st.set_page_config(page_title="Biotech Financial Model", layout="wide")
    ...
    product_df = st.data_editor(
        ...
        column_config={
            ...
            "success_prob": st.column_config.NumberColumn("Success probability", min_value=0.0, max_value=1.0, step=0.05),
        },
    )
    ...
    new_money = vc_col4.number_input("New money ($)", min_value=1_000_000, value=50_000_000, step=5_000_000)
    ...
    with st.expander("Optional: train an ML multiples model"):
        ...
        if uploaded is not None:
            ...
            if len(train_df.columns) >= 2:
                ...
                st.warning("Upload a dataset with at least one feature column plus the target.")
