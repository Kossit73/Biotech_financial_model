    ...
    with config_tab:
        with _section_block(
            ...
            with st.expander("Risk-adjusted DCF valuation method - assumptions"):
                ...
                with col_b:
                    current_discount_timing = str(
                        getattr(draft_defaults, "discount_timing", "year_end") or "year_end"
                    )
                    if current_discount_timing not in {"year_end", "mid_year", "year_0"}:
                        current_discount_timing = "year_end"
                    discount_timing = st.selectbox(
                        "Discount timing",
                        options=["year_end", "mid_year", "year_0"],
                        index=["year_end", "mid_year", "year_0"].index(current_discount_timing),
                ...
                with dcf_cols[0]:
                    current_terminal_method = str(
                        getattr(draft_defaults, "terminal_method", "exit_multiple") or "exit_multiple"
                    )
                    if current_terminal_method not in {"exit_multiple", "perpetuity_growth"}:
                        current_terminal_method = "exit_multiple"
                    terminal_method = st.selectbox(
                        "Terminal method",
                        options=["exit_multiple", "perpetuity_growth"],
                        index=["exit_multiple", "perpetuity_growth"].index(current_terminal_method),
                        format_func=lambda option: {
